...
5. **Question** I don't care about smooth movement. I want the character to only show on the true tile, without interpolation. Can I do this? **Answer**: In the configuration, set the duration to 0ms. You can also set the rotation time to 0ms if you want rotations to be instant too.
6. **Question** Should I use this instead of a true tile plugin? **Answer**: This isn't necessarily meant to replace true tiles. You can happily use both, though true tile plugins become less necessary when the model lines up better.
7. **Question** How does this compare to Posiedien's True Tile Animations plugin? **Answer**: They are similar plugins and, in fact, we've both collaborated a bit on both. They have very different scopes, however. True Tile Animations is supposed to completely rework the animation system. This plugin tries to touch as little as possible except for the player's visual position, which should make it a bit more stable and future-proof as well. It's personal preference which one you should go with, and I encourage you to try out both.
